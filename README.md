...
- **拖入即用**：拖入 / 粘贴 / 选择 PNG、JPG、WebP 图片，自动识别抠图方式（透明通道 / 去黑底 / 去白底），抠图容差可调，轻松应对 JPEG 噪底
- **自由透视**：鼠标拖动即可旋转、缩放、平移视角，可调视场角
- **噪波能量**：FBM 分形噪波控制魔法阵明暗流动，强度 / 缩放 / 速度 / 细节层数 / 对比度可调
- **波动扰动**：旗帜式顶点波动，让魔法阵产生轻微上下浮动的呼吸感
- **影视级辉光**：UnrealBloom + ACES 色调映射，强度 / 半径 / 阈值 / 曝光可调；内置保色软限幅与半径权重归一，参数拉满也不会死白爆屏
- **多种导出**：
...
1. **贴图**：拖入一张魔法阵图片（推荐带透明通道的 PNG；纯黑底 / 纯白底的图片会自动识别并切换抠图方式）。可换颜色或勾选「保留贴图原色」；若底色抠不干净（如 JPEG 近黑噪底），加大「抠图容差」即可
...
- **three.js**（v0.160.1，已内置于 `vendor/`，MIT 协议；其中 UnrealBloomPass / LuminosityHighPassShader 有少量定制：辉光输入做保色软限幅、合成权重归一并截断负权重，防止大半径 / 大强度下死白爆屏）
